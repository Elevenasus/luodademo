 ...
 */
public class ACL_5_INTERNAL_PROGRAM_TINY_PEQ extends AclCmd implements IAclHandleResp {
    private final String TAG = "INTERNAL_PROGRAM_PEQ";
    ...
    private byte[] mPeqHpfStruct;

    public ACL_5_INTERNAL_PROGRAM_TINY_PEQ(AirohaOtaFlowMgr mgr) {
        super(mgr);
    }

    public void setPeqHpfStruct(byte[] struct){
        mPeqHpfStruct = struct;
    }
...
//    @Override
    private byte[] getCommand() {
        ...
        Log.d(TAG, "flash addr: " + mProgramStartAddr);
...
//        }
    ...
    private void PrepareCmdListForTinyBin(){
        ...
        try {
            ...
            ios = new ByteArrayInputStream(mPeqHpfStruct);
            ...
            while (ios.read(buffer) != -1) {
                if(!ContentChecker.isAllDummyHexFF(buffer)) {
                    ...
                    System.arraycopy(buffer, 0, cmd, 11, buffer.length);
    ...
    public void ParsePacket(byte[] packet) {
        ...
        Log.d(TAG, " addr:" + "" + b2 + " " + b1 + " " + b0 + " , status: " + status);

        if (status == (byte) 0x00 &&
                b2 == cmd[7] && b1 == cmd[8] && b0 == cmd[9]) {
            isCmdPass = true;

            Log.d(TAG, "response index: " + mResponseIdx);
    ...
    public void setCheckFwSupported() {
    }
}